...
void PluginLivoxCustom::OnNewLaserScans() {
    if (rayShape) {
        ...
        // sensor_msgs::PointCloud2Modifier modifier(livox_msg);
        // modifier.setPointCloud2FieldsByString(1, "xyz");
        // modifier.resize(points_pair.size());

        // sensor_msgs::PointCloud2Iterator<float> iter_x(livox_msg, "x");
        // sensor_msgs::PointCloud2Iterator<float> iter_y(livox_msg, "y");
        // sensor_msgs::PointCloud2Iterator<float> iter_z(livox_msg, "z");
        ...
        for (auto &pair : points_pair) {
            ...
            //   auto index = (verticalRayCount - verticle_index - 1) * rayCount + horizon_index;
                ...
                // livox_msgs.emplace_back();
                // livox_msg.points.emplace_back(0, point.X(), point.Y(), point.Z(), 0, 0, 0);
                (*iter).x=point.X();
                (*iter).y=point.Y();
                (*iter).z=point.Z();
                ++iter;
